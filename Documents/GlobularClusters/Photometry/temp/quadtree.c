...
source_t *new_source(double flux_aper, double mag_aper, double magerr_aper, double x_image, double y_image,
    ...
    source->x_world = x_world;
    source->y_world = y_world;
    source->alpha = alpha;
    source->delta = delta;
    source->ellipticity = ellipticity;
    source->fwhm_image = fwhm_image;
    source->fwhm_world = fwhm_world;
    source->flags  = flags;
    source->class_star = class_star;
    return source;
}
...
void insert_source(node_t *node, source_t *source) {
    ...
    if (node->q1 != NULL) {
        ...
        if (source->x_image >= node->xmid) {
            if (source->y_world >= node->ymid)
                quadrant = node->q1;
            else
                quadrant = node->q4;
        } else {
    ...
    if (intersecting(&node->box, interest)) {
        ...
        if (node->q1 == NULL) {
            ...
            for (s = (source_t *)node->contents.first; s != NULL; s = (source_t *)s->links.next) {
                s_dist = norm2(s->x_world, s->y_world, tx, ty);
                if (debug)
                    printf("  comparing (%0.2f, %0.2f) dist %0.4f", s->x_world, s->y_world, s_dist);
                if (s_dist < *dist) {
                    *nearest = s;
                    *dist = s_dist;
